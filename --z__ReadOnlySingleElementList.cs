...
internal sealed class _003C_003Ez__ReadOnlySingleElementList<T> : IEnumerable, ICollection, IList, IEnumerable<T>, IReadOnlyCollection<T>, IReadOnlyList<T>, ICollection<T>, IList<T>
{
    private sealed class Enumerator : IDisposable, IEnumerator, IEnumerator<T>
    {
        ...
        object IEnumerator.Current => (object)_item!;
    ...
    bool IList.Contains(object? value)
    {
        if (value is T t)
        {
            return EqualityComparer<T>.Default.Equals(_item, t);
        }
        return value == null && EqualityComparer<T>.Default.Equals(_item, default(T));
    }
    ...
    int IList.IndexOf(object? value)
    {
        if (value is T t)
        {
            return EqualityComparer<T>.Default.Equals(_item, t) ? 0 : -1;
        }
        return (value == null && EqualityComparer<T>.Default.Equals(_item, default(T))) ? 0 : -1;
    }
