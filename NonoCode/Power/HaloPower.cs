...
public sealed class HaloPower : NonoPower
{
    ...
    [
        new DynamicVar("RealDamage", 1m)
    ];
    public void SetDamage()
    {
        AssertMutable();
        DynamicVars["RealDamage"].BaseValue = Amount ;
    }
    //定义一个方法来设置伤害数值，伤害数值为能力数值
    public override IEnumerable<HealthBarForecastSegment> GetHealthBarForecastSegments(HealthBarForecastContext context)
    {
        return new List<HealthBarForecastSegment>
        {
            new HealthBarForecastSegment(Amount * 3 , new Color(0.9843f, 1.0f, 0.0f, 1.0f), 0, 0)
        };
    }
